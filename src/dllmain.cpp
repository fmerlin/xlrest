...
extern "C" {
	...
	__declspec(dllexport) int WINAPIV xlAutoOpen(void) {
		rms::logger = new rms::Log();
		rms::logger->Init(g_hinstance, std::string("%TEMP%\\" APPNAME ".log"), std::string("debug"));
		try {
			rms::config = new rms::Config();
			rms::logger->Debug("Init " APPNAME);
			rms::config->init3(g_hinstance);
			rms::logger->Debug("Starting " APPNAME);
		}
	...
	__declspec(dllexport) int WINAPIV xlAutoClose(void)
	{
		if(rms::logger)
			rms::logger->Debug("Closing " APPNAME);
		rms::config->close();
		delete rms::logger;
		rms::logger = NULL;
		return 1;
	...
	__declspec(dllexport) LPXLOPER12 WINAPIV xlAutoRegister12(LPXLOPER12 pxName)
	{
	...
	__declspec(dllexport) int WINAPIV xlAutoAdd(void)
	{
		return 1;
	}

	__declspec(dllexport) int WINAPIV xlAutoRemove(void)
	{
		return 1;
	}

	void freeXL(LPXLOPER12 xl) {
		switch (xl->xltype) {
		case xltypeStr:
			free(xl->val.str);
			break;
		case xltypeMulti:
			int n = xl->val.array.rows * xl->val.array.columns;
			for (int i = 0; i < n; i++)
				freeXL(&xl->val.array.lparray[i]);
			free(xl->val.array.lparray);
			break;
		}
	}

	__declspec(dllexport) void WINAPIV xlAutoFree12(LPXLOPER12 px)
	{
		if (!px) return;
		freeXL(px);
		free(px);
	...
	__declspec(dllexport) LPXLOPER12 WINAPIV xlAddInManagerInfo12(LPXLOPER12 xAction)
	{
		...
		if (xIntAction.val.w == 1) {
			...
			x->val.str = (wchar_t*)malloc(32);
			x->val.str[0] = (unsigned char)strlen(APPNAME);
			wsprintf(&x->val.str[1], L"%s", APPNAME);
			return x;
		}
...
__declspec(dllexport) LPXLOPER12 WINAPIV FS##number (LPXLOPER12 v0, LPXLOPER12 v1, LPXLOPER12 v2 \
...
{ \
	return rms::config->execute(number, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15, v16, v17, v18, v19); \
} 
